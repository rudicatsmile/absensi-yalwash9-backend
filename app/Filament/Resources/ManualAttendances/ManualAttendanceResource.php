                ...
                Action::make('update')
                    ...
                    ->action(function (User $record, array $data, \Livewire\Component $livewire) {
                        ...
                        try {
                            ...
                            // Update time_out and latlon_out
                            if (isset($data['time_out'])) {
                                $att->time_out = is_string($data['time_out']) ? $data['time_out'] . ':00' : $data['time_out'];

                                // Set latlon_out consistent with latlon_in or location
                                if (!empty($att->latlon_in) && $att->latlon_in !== '-') {
                                    $att->latlon_out = $att->latlon_in;
                                } elseif ($att->company_location_id) {
                                    $loc = \App\Models\CompanyLocation::find($att->company_location_id);
                                    if ($loc) {
                                        $att->latlon_out = "{$loc->latitude},{$loc->longitude}";
                                    } else {
                                        $att->latlon_out = '-6.1914783,106.9372911';
                                    }
                                } else {
                                    $att->latlon_out = '-6.1914783,106.9372911';
                                }
                            ...
                            Log::error('manual_attendance.update_failed: ' . $e->getMessage());
                            Notification::make()
